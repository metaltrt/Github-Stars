...
public abstract class NetworkBoundResource<T, V> {
    ...
    private String getCustomErrorMessage(Throwable error) {
        ...
        if (error instanceof SocketTimeoutException) {
            return GithubStarsApp.getAppContext().getString(R.string.requestTimeOutError);
        } else if (error instanceof MalformedJsonException) {
            return GithubStarsApp.getAppContext().getString(R.string.responseMalformedJson);
        } else if (error instanceof IOException) {
            return GithubStarsApp.getAppContext().getString(R.string.networkError);
        } else if (error instanceof HttpException) {
            return (Objects.requireNonNull(((HttpException) error).response()).message());
        } else {
            return GithubStarsApp.getAppContext().getString(R.string.unknownError);
        }
